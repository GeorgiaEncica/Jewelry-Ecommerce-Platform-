...
<head>
<meta charset="UTF-8">
<title>Earrings | Âme Jewelry</title>
...
<h1 class="category-title">Earrings Collection</h1>
...
<div class="products-container" id="products-container">
    <div class="loading-overlay"><i class="fas fa-spinner"></i><p>Loading Earrings...</p></div>
</div>
...
function loadProducts() {
    productsContainer.innerHTML = '<div class="loading-overlay"><i class="fas fa-spinner"></i><p>Loading Earrings...</p></div>';
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                resultsInfo.innerHTML = `Showing ${data.count} earring${data.count !== 1 ? 's' : ''}`;
            }
        })
        .catch(err => {
            console.error(err);
...
function displayProducts(products) {
    if (!products.length) {
        productsContainer.innerHTML = '<p style="text-align:center;color:#999;">No earrings found.</p>';
        return;
    }
