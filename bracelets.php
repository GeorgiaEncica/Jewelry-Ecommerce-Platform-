...
<head>
<meta charset="UTF-8">
<title>Bracelets | Âme Jewelry</title>
...
<h1 class="category-title">Bracelets Collection</h1>
...
<div class="products-container" id="products-container">
    <div class="loading-overlay"><i class="fas fa-spinner"></i><p>Loading Bracelets...</p></div>
</div>
...
function loadProducts() {
    productsContainer.innerHTML = '<div class="loading-overlay"><i class="fas fa-spinner"></i><p>Loading Bracelets...</p></div>';
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                resultsInfo.innerHTML = `Showing ${data.count} bracelet${data.count !== 1 ? 's' : ''}`;
            }
        })
        .catch(err => {
            console.error(err);
...
function displayProducts(products) {
    if (!products.length) {
        productsContainer.innerHTML = '<p style="text-align:center;color:#999;">No bracelets found.</p>';
        return;
    }
